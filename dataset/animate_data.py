...
def main():
    ...
    amsr2_path = '/lustre/storeB/project/metkl/DigitalSeaIce/are-phd/CouplingProject/Dataset/amsr2/'
    ...
    with Dataset(f"{aa_path}2025/01/01/arome_arctic_det_2_5km_20250101T00Z.nc", 'r') as constants:
    ...
    buoy_kvs_10_idx_time_temp = reference_time_temp + dataset['time_temp'][buoy_kvs_10_idx, :].astype('timedelta64[s]')
    buoy_kvs_10_t1m = dataset['temp_air_raw'][buoy_kvs_10_idx, :]
    buoy_kvs_10_temp_df = pd.DataFrame({'t1m': buoy_kvs_10_t1m}, index = buoy_kvs_10_idx_time_temp).resample('60min').mean()

    
    reference_time_traj = np.datetime64(dataset['time'].units[14:])
    buoy_kvs_10_time_traj = reference_time_traj + dataset['time'][buoy_kvs_10_idx, :].astype('timedelta64[s]')

    buoy_kvs_10_traj_df = pd.DataFrame({'lat': dataset['lat'][buoy_kvs_10_idx, :], 'lon': dataset['lon'][buoy_kvs_10_idx, :]}, index=buoy_kvs_10_time_traj).resample('60min').mean()

    full_df = pd.merge(buoy_kvs_10_traj_df, buoy_kvs_10_temp_df, left_index=True, right_index=True)
    ...
    def create_animation():
        ...
        def animate(frame):
            ...
            if start_idx < len(full_df):
                ...
                try:
                    new_ice = Dataset(f"{amsr2_path}{new_time.year}/{new_time.month:02d}/RegridAMSR2_{new_time.strftime('%Y%m%d')}.nc")['amsr2'][:]
                ...
                ice_pcolormesh = ax.pcolormesh(lon, lat, new_ice, cmap = cmocean.cm.ice, transform = data_proj, norm = norm_ice, zorder = 0)

                new_scatter = ax.scatter(new_lons, new_lats, c = new_temps, cmap = cmocean.cm.thermal, norm = norm, transform = data_proj, zorder = 1)

                scatter_plots.append(new_scatter)

            # total_points = min(end_idx, len(full_df))
            ax.set_title(f"Buoy KVS-10 Track - {new_time.strftime('%Y-%m-%d')}", fontsize = 14, pad = 20)

            # return scatter_plots
            return [ice_pcolormesh] + scatter_plots if ice_pcolormesh is not None else scatter_plots
